...
const signup = async (req, res) => {
     ...
     try {
          ...
          if (password.length < 8) return res.status(201).json({ code: 10, msg: "Password Length Must Be Minimum 8 Character" })
          if (!userName || !email || !password) return res.status(400).json({ code: 11, msg: "All Field Are Required" })
          if (userExist) return res.status(201).json({ code: 12, msg: "User Already Exist" })
